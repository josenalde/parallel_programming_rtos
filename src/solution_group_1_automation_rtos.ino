// tarefa executada em laboratório@eaj@ufrn 2023.2 por
// Francisco Emerson, Emanuel Italo, Ramonie, Luiz Sousa
...
void taskControlLed(void *pvParameters) {
  ...
  while (1) {
    ...
    if (xQueueReceive(queueVoltage, &voltage, portMAX_DELAY)) {
      ...
      dacWrite(ledPin, map(sensor, 0, 4095, 0, 255));
      xSemaphoreGive(voltageSemaphore);
    }
  }
}
...
void setup() {
  ...
  // Inicializando o temporizador auto-reload
